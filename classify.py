...
def predicting():
	for filename in os.listdir(abs_dir):
		...
		if allowed_file(filename):
			...
			abs_file = abs_dir+seperator+filename
			...
			if os.path.isfile(abs_file) == True:
				...
				if filename.rsplit('.', 1)[1].lower() == 'mp4' or filename.rsplit('.', 1)[1].lower() == 'mov':
					...
					for frame in response['outputs'][0]['data']['frames']:
						...
						for concept in frame['data']['concepts']:
							...
							if concept['name'] not in nullifying_tags and concept['name'] not in nature_tags:
								if concept['name'] in dict1:
									dict1[concept['name']]+= 1
								else:
									dict1[concept['name']]= 1
				...
				print(len(tags),len(nullifying_tags_found),len(total))
				if len(tags)>len(nullifying_tags_found) or len(tags)/len(total) >=0.5:
					print("Nature or weather photo because these tags: "+str(tags)+" were more than the nullifying tags: "+str(nullifying_tags_found)+" percentage: "+str(len(tags)/len(total))+"\n\n\n\n")
					shutil.move(abs_file, nature_dir)

				else:
					if len(tags)<=len(nullifying_tags_found):
						print("Not a nature photo because these tags: " +str(nullifying_tags_found)+" percentage: "+str(len(tags)/len(total))+"\n\n\n\n")
					else:
						print("Not a nature photo because no nature tags were found(there were no nullifying tags)\n\n\n\n")
...
for key in keys:
	...
	if len(os.listdir(abs_dir)) ==1 and os.listdir(abs_dir)[0]=='.DS_Store' or len(os.listdir(abs_dir))==0:
		...
		break
	if status == True:
		print("The key being used is " + key +"\n")
		response = predicting()
